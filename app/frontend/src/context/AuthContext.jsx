...
export const AuthProvider = ({ children }) => {
	...
	const initialUser =
		authTokens?.access_token && jwtDecode(authTokens.access_token);
	...
	const refres_token = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				Cookies.set(
					'access_token',
					JSON.stringify(updatedTokens.access_token),
				);
				Cookies.set(
					'refresh_token',
					JSON.stringify(updatedTokens.refresh_token),
				);
			} else {
				console.log('--- Login failed ---', data);
				logout();
	...
	useEffect(() => {
		if (authTokens) {
			try {
				const accessTokenExpirationTime = get_expiration_time(
					authTokens.access_token,
				);
				const refreshTokenExpirationTime = get_expiration_time(
