...
function NotificationDropdown() {
	...
	const handleAcceptFriendRequest = async (friendRequestId) => {
		...
		try {
			const response = await fetch(
				`http://127.0.0.1:8000/api/friend_request/accept/${friendRequestId}/`,
				{
					method: 'POST',
	...
	const handleCancelFriendRequest = async (friendRequestId) => {
		...
		try {
			const response = await fetch(
				`http://127.0.0.1:8000/api/friend_request/cancel/${friendRequestId}/`,
				{
					method: 'POST',
					headers: getAuthHeaders(),
				}
	...
	return (
		<>
			...
			<div className='flex-1 flex flex-col tb:gap-3 gap-2 overflow-auto lp:mx-3 mx-2 mb-2 font-medium'>
				{notifications.length > 0 ? (
					notifications.map((notification, index) => (
						<React.Fragment key={index}>
							<div className='flex items-center justify-between notification-img'>
