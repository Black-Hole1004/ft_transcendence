...
function NotificationDropdown({
	...
	const handleAcceptFriendRequest = async (friendRequestId) => {
		...
		try {
			const response = await fetch(
				`${ACCEPT_FRIEND_REQUEST}${friendRequestId}/`,
				{
					method: 'POST',
			...
			if (response.status === 201) {
				...
				setNotifications((prevNotifications) =>
					prevNotifications.filter(
						(notification) => notification.id !== friendRequestId,
					),
				);
	...
	const handleCancelFriendRequest = async (friendRequestId) => {
		...
		try {
			const response = await fetch(
				`${CANCEL_FRIEND_REQUEST}${friendRequestId}/`,
				{
					method: 'POST',
					headers: getAuthHeaders(),
				},
	...
	useEffect(() => {
		const fetchNotifications = async () => {
			try {
				const response = await fetch(
					`${BASE_URL}/api/friend_ship_request/`,
					{
						headers: getAuthHeaders(),
					},
				);
							...
							<div className="flex items-center justify-between">
								<div className="flex items-center gap-2">
									<img
										...
										className="mtb:border border-0.7 aspect-square object-cover border-primary rounded-full notification-img"
										alt="User Avatar"
									/>
									{notification.flag ? (
										<p>
											{notification.username} sent you a
											Friend Request!
										</p>
									) : (
										<p>
											{notification.from_user} sent you a
											Friend Request!
										</p>
									)}
								</div>
