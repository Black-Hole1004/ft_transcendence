...
function User({
	...
	if (ids)
		selectedUserId = ids[0] === currentLoggedInUserId ? ids[1] : ids[0];
	...
	const getTimePassed = () => {
		...
		let timePassed =
			(Date.parse(currentTime) - Date.parse(messageTime)) / 1000;
	...
	return (
		...
		>
			...
			<div className="font-medium tb:w-[80%]">
				...
				{search ? (
					<p className="text-level last-message max-tb:hidden">
						celestial master
					</p>
				) : (
