...
const GameOverPopup = ({
	...
	const profileImageUrl = getFullImageUrl(
		currentPlayerStats?.profile_picture,
	);
	...
	useEffect(() => {
		const timer = setTimeout(() => {
			setProgressWidth(
				currentPlayerStats?.badge?.progress_percentage || 0,
			);
		}, 500);
				...
				<div className="flex flex-col gap-2">
					...
					<div className="flex items-center gap-4">
						...
						<div className="flex flex-col items-start">
							<h2 className="font-heavy text-primary">
								{currentPlayerStats?.username}
							</h2>
							<p className="achievement-name font-dreamscape-sans text-level">
								{currentPlayerStats?.badge?.name}
							</p>
							<p className="font-medium text-border leading-none">
					...
					<div className="bg-border bg-opacity-20 rounded p-4">
						<h3 className="text-primary font-heavy mb-4 text-start">
							Current Achievement
						</h3>
						<div className="flex items-center gap-4">
							<div>
