	...
	useEffect(() => {
		const fetchUserData = async () => {
			try {
				console.log(
					'API URL:',
					`${BASE_URL}/api/users/${user.profile_name}/time-spent/`,
				);
				...
				if (response.ok && Array.isArray(result.data)) {
					...
					const apiData = result.data.map((item) => {
						...
						const minutes = item.total_time_spent_seconds
							? Math.round(item.total_time_spent_seconds / 60)
							: 0;
