...
class GameWebSocket {
	...
	handleOpen() {
		console.log(
			`Connected to game ${this.gameId} through WebSocket successfully`,
		);
	...
	handleClose(event) {
		...
		if (!event.wasClean && this.connectionAttempts < this.maxRetries) {
			console.log(
				`Attempting reconnect ${this.connectionAttempts + 1}/${this.maxRetries}`,
			);
	...
	handleMessage(event) {
		try {
			...
			const handlers = {
				...
				player_number_confirmed:
					this.handlePlayerNumberConfirmed.bind(this),
	...
	sendPaddleMove(action) {
		// action will be 'startUp', 'startDown', 'stopUp', or 'stopDown'
		this.send({
