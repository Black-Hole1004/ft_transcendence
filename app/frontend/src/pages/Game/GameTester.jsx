...
const GameTester = ({ gameId }) => {
	...
	const interpolateBallPosition = useCallback(() => {
		const INTERPOLATION_FACTOR = 0.70;
		...
		interpolationFrame.current = requestAnimationFrame(interpolateBallPosition);
	}, []);
	...
	const movePaddle = useCallback(() => {
		...
		let currentY =
			playerNumber === 1 ? gameState.player1_paddle?.y || 0 : gameState.player2_paddle?.y || 0
		let newY = currentY
		...
		<div className='p-4 bg-gray-900 text-white min-h-screen'>
			<div className='max-w-7xl mx-auto'>
				...
				<div className='mb-4 text-center'>
					<p>Pauses Remaining - P1: {pausesRemaining[1]} P2: {pausesRemaining[2]}</p>
				...
				<div className='flex gap-4 mb-6'>
					...
					>
						{isPaused ? 'Resume Game' : `Pause Game (${pausesRemaining[playerNumber]} left)`}
					</button>
				</div>
