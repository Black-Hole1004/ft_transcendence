...
class MatchmakingConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if hasattr(self.scope.get('user'), 'username'):
            print(f"Username: {self.scope.get('user').username}")
        
        self.user = self.scope.get('user') # chaged to Safely get user from scope
    ...
    async def disconnect(self, close_code):
        ...
        print("Disconnecting from matchmaking service pnoxr")
        try:
            if self.channel_name in self.matchmaking_queue:
                user = self.matchmaking_queue[self.channel_name].get('user')
                if user:
                    print(f"{user.username} disconnecting from matchmaking service")
