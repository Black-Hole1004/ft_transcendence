...
class GamePhysics:
    ...
    def update_ball_position(self, ball, delta_time):
        ...
        next_x = ball['x'] + ball['velocityX'] * ball['speed'] * delta_time
        next_y = ball['y'] + ball['velocityY'] * ball['speed'] * delta_time
        ...
        # Check scoring/side walls
        if next_x + ball['radius'] >= self.canvas_width or next_x - ball['radius'] <= 0:
            # Reset ball to center with new random angle
            self.reset_ball(ball, direction=-1 if next_x <= 0 else 1)
            return
...
class GameConsumer(AsyncWebsocketConsumer):
    def __init__(self, *args, **kwargs):
        ...
        self._game_id = None  # Add this line

    @property
    def game_id(self):
        if not self._game_id:
            # Try to get game_id from scope if not already set
            self._game_id = str(self.scope['url_route']['kwargs'].get('game_id'))
    ...
    async def connect(self):
        ...
        try:
            self.game_id = str(self.scope['url_route']['kwargs']['game_id'])  # This will use the setter
            self.room_group_name = f'game_{self.game_id}'
            
            # Get or create game state
            try:
                print("Attempting to get game state from cache")  # Debug print
                game_state = cache.get(f'game_state_{self.game_id}')
                print(f"Retrieved game state: {game_state}")  # Debug print
                if game_state is None or not hasattr(game_state, 'is_session_expired') or game_state.is_session_expired():
                    print("Creating new game state")  # Debug print
            ...
            # check if the game is full before incrementing the connected players
            ...
            while not self.game_state.game_over and self.game_state.connected_players == 2:
                ...
                if not self.game_state.is_paused:
                    ...
                    if current_time - last_physics_time >= physics_interval:
                        # Use shorter sleep intervals to allow other events to be processed
                        await asyncio.sleep(0.001)  # Very short sleep to allow other tasks
                    ...
                    # update the cache with the latest game state
                    try:
                        with cache.lock(f'game_state_{self.game_id}_lock'):
                            cache.set(f'game_state_{self.game_id}', self.game_state)
                    except Exception as e:
                        print(f"Error updating cache in game loop: {str(e)}")
                    # State broadcast
    ...
    async def handle_paddle_move(self, data):
        ...
        print("\n=== Handling Paddle Move ===")
        print("Current game state before move:")
        print(f"P1 y: {self.game_state.player1_paddle['y']}")
        print(f"P2 y: {self.game_state.player2_paddle['y']}")
        
        # Restore room_group_name if missing
        if not hasattr(self, 'room_group_name'):
            self.room_group_name = f'game_{self.game_id}'
            print(f"Restored room_group_name: {self.room_group_name}")

        # Always get fresh state from cache first
        try:
            cached_state = cache.get(f'game_state_{self.game_id}')
            if cached_state:
                self.game_state = cached_state  # Update local state with cached state
                print("Retrieved fresh state from cache")
        except Exception as e:
            print(f"Error getting state from cache: {e}")

        if not self.game_state:
            print("No game state available! even in the cache")
            return
        ...
        try:
            ...
            if self.player_number == 1:
                ...
                print(f"Updated player 1 paddle to y={new_y}")
            else:
                self.game_state.player2_paddle['y'] = new_y
                print(f"Updated player 2 paddle to y={new_y}")
            ...
            try:
                ...
                print(f"Error updating cache in handle_paddle_move: {str(e)}")
            # Prepare message
            message = {
                'type': 'paddles_update',
                'paddles': {
    ...
    async def handle_player_ready(self, data):
        try:
            ...
            cached_state = cache.get(f'game_state_{self.game_id}')
            if cached_state:
                cached_state.players_ready.add(self.player_number)
                self.game_state = cached_state
            else:
                self.game_state.players_ready.add(self.player_number)

            print(f"Current players ready: {self.game_state.players_ready}")
    ...
    async def handle_pause_game(self, data):
        ...
        try:
            # First check if game_state exists
            if not self.game_state:
                await self.send(json.dumps({
                    'type': 'error',
            ...
            # Now we can safely check game_over
    ...
    async def handle_start_game(self, data):
        ...
        try:
            ...
            if self.game_state.connected_players == 2 and not self.game_state.game_over and len(self.game_state.players_ready) == 2:
                self.game_state.is_paused = False
                
                # Start game loop if not already running
                if not self.game_loop_task:
                    print("Starting game loop")
                    # Use create_task to run the game loop concurrently
                    self.game_loop_task = asyncio.create_task(self.game_loop())
                    # Don't await it here - let it run independently
                    
                print("Game started")
                
            else:
                print(f"Can't start game: players={self.game_state.connected_players}, game_over={self.game_state.game_over}, players_ready={self.game_state.players_ready}")
    ...
    async def handle_scoring(self, scorer):
        ...
        #print two new lines to separate the ball's coordinates
        print("\n\n")
        print(f"A GOAAAAAAL! Player {scorer} scored!")
    ...
    async def broadcast_game_state(self):
        ...
        # Get fresh state from cache first
        try:
            cached_state = cache.get(f'game_state_{self.game_id}')
            if cached_state:
                self.game_state = cached_state
        ...
        current_state = {
            ...
            # Don't include paddle positions in regular state updates
        }
        
        print(f"Broadcasting ball state: {current_state}")
        
        await self.channel_layer.group_send(
            self.room_group_name,
            {
                'type': 'ball_update',  # New message type for ball only
                'state': current_state
            }
        )
    ...
    async def paddles_update(self, event):
        """Handle paddle updates and forward to client"""
        await self.send(text_data=json.dumps({
            'type': 'paddles_update',
    ...
    async def player_ready(self, event):
        """Forward player ready status to client"""
        await self.send(text_data=json.dumps({
            'type': 'player_ready',
            'player': event['player']
        }))
    ...
    async def game_started(self, event):
        """Forward game started status to client"""
        await self.send(text_data=json.dumps({
            'type': 'game_started',
    ...
    async def handle_game_over(self):
        ...
        print(f"The winner is {self.game_state.winner}")
        print(f"Player 1 score: {self.game_state.player1_paddle['score']}")
        print(f"Player 2 score: {self.game_state.player2_paddle['score']}")
